...
public abstract class Script_Instance_bd3b5 : GH_ScriptInstance
{
    ...
    private void RunScript(List<Point3d> Points, List<double> Radius, ref object OutTemp)
    {
        ...
        double maxTangentArcRadius = 1;
        double minTangentArcRadius = 0.0001;
        bool guide = false;
        ...
        for (int i = 1; i < Points.Count - 1; i++)
        {
            ...
            if (l0 == null || l1 == null || Points[i].DistanceTo(Points[i + 1]) < Radius[i] + Radius[i + 1])
            {
        ...
        //   curves.Add(new Arc(curves[i].PointAtEnd, curves[i].TangentAtEnd, curves[i + 1].PointAtStart).ToNurbsCurve());
        //   // curves.Add(tangents[i]);
        // }
    ...
    void CircleCircleIntersection(Point3d p0, double r0, Point3d p1, double r1, out Point3d intersection0, out Point3d intersection1)
    {
        ...
        double h1 = r1 * cosA;
        double t1 = h1 / d;
        Point3d midPt = p0 * (1 - t1) + p1 * t1;

        double h = Math.Sqrt(Math.Pow(r0, 2) - Math.Pow(p0.DistanceTo(midPt), 2)); //CE

        Vector3d normal = new Vector3d(p0 - p1);
        normal = new Vector3d(1, -normal.X / normal.Y, normal.Z);
        normal.Unitize();
        normal *= h;

        intersection0 = midPt + normal;
        intersection1 = midPt - normal;
    ...
    void getTangentLineCurves(Point3d p0, double r0, Point3d p1, double r1, out NurbsCurve l0, out NurbsCurve l1)
    {
        ...
        l0 = new Line(r0_low_point, r1_up_point).ToNurbsCurve();
        l1 = new Line(r0_up_point, r1_low_point).ToNurbsCurve();
