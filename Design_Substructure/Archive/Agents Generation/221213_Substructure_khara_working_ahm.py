...
class Environment(object):
    ...
    def __init__(self, u_div, v_div, surface, agents_list = [], repulsion_thresh = 0.5):
        ...
        self.strongAgents = self.GenerateStrongAgentsList()
    ...
    def update_agents_pos (self, coherence_rad, coherence_fac, align_rad, align_fac, avoid_rad, avoid_fac, Coherence_T, Alignment_T, Separation_T, repulsion_rad = 0, repulsion_fac = 0, Repulsion_T = False):
        ...
        for agent in self.agents:
            ...
            if Repulsion_T and agent.unitized_upForce > self.repulsion_thresh: #we can apply repulsion
                repulsion_vector = agent.Separation(repulsion_rad, self.strongAgents, self.u_div, self.v_div, repulsion_fac)
                if repulsion_vector.X > 0 and repulsion_vector.Y > 0:
                    effects_vector += repulsion_vector
                    effects_list.append(agent.UnitizeEffect(self.u_div, self.v_div, effects_vector))
                    print ("we are here")
                    continue
...
combined_env = Environment(u_div, v_div, surface, all_agents, repulsion_thresh)
for t in range(time_2):
    combined_env.update_agents_pos(coherence_rad, coherence_fac, align_rad, align_fac, avoid_rad, avoid_fac, Coherence_T, Alignment_T, Separation_T, repulsion_rad, repulsion_fac, Repulsion_T)
